    ...
    <title>
       <?php	
		...
		if(isset($_SESSION["album"])){
		   ...
		   foreach($r2 as $album){
			   $r=$album;
			   echo $r->getTitle().' Photos';
		  }
	   }
     ?>
    </title>
    <link rel="stylesheet" href="../CSS/FormatMadeInTheAMPhoto.css">
  </head>
  ...
  <body>
   ...
   ?>
    ...
    <?php
	   ...
	   $p=new Photo();
	   $photoArray=$p->getPhotosAccordingToReleaseId($_SESSION["album"]);
	   
	   foreach($photoArray as $photo){
		   $p=$photo;
		   echo '<div id="photo">';
		   $source="../Images/Releases Photos/".$p->getPhoto();
		   echo '<img src="'.$source.'" alt="'.$p->getPhoto().'" width="100%" height="auto">';
		   echo '<h1>'.$p->getPhotoTitle().'</h1>';
		   echo '</div>';
	   }  
