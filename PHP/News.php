...
<html>
  <head>
    <meta charset="utf-8">
    <title>News</title>
    <link rel="stylesheet" href="../CSS/FormatNews.css">
  ...
  <body>
   ...
   ?>
    ...
    <?php
	   ...
	   $news_per_page=9;
	   $totalNumberOfRecords=$n->getTotalNumberOfNews();
	
	   //get total number of pages
	 
	   $numOfPages=ceil($totalNumberOfRecords/$news_per_page);
	   ...
	   $left_rec = $totalNumberOfRecords-($page * $news_per_page);
	   
	   $newsArray=$n->getAllTheNews($offset,$news_per_page);	   
	  
	   
	   foreach($newsArray as $news){
		   $n=$news;
		   echo '<div id="news"><a href="NewsDescription.php?news='.$n->getNewsId().'" class="hvr-grow">';
		   $source="../Images/News/".$n->getImage();
		   echo '<img src="'.$source.'">';
		   echo '<h2>'.$n->getTitle().'</h2></a>';
		   echo '</div>';
	   }
	  ...
	  if( $page > 0 && $realPageNo<$numOfPages) {
		  ...
		  echo "<a href = \"$_PHP_SELF?page=".$page."\" class=\"links\">NEXT</a>";
	  }else if( $left_rec <=$news_per_page && $news_per_page<$totalNumberOfRecords) {
		  $last = $page - 2;
		  echo "<a href = \"$_PHP_SELF?page=".$last."\" class=\"links\">PREVIOUS</a>";
	  }	
	?>
