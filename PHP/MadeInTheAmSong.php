    ...
    <title>
       <?php	
		...
		if(isset($_SESSION["album"])){
		   ...
		   foreach($r2 as $album){
			   $r=$album;
			   echo $r->getTitle().' Songs';
		  }
	   }
     ?>
    </title>
    <link rel="stylesheet" href="../CSS/FormatMadeInTheAMSongs.css">
  ...
  <body>
   ...
   ?>
    ...
    <?php
	   ...
	   if(isset($_POST["btn_lyric"])){
		   $_SESSION["song"]= $_POST["btn_lyric"];
	       header('location:LyricsOfPerfect.php');
	   }	  
	   ...
	   $s=new Song();
	   $songArray=$s->getSongsAccordingToReleaseId($_SESSION["album"]);
	   
	   $so=new SongImage();
	   $songImageArray=array();
	   
	   foreach($songArray as $song){
		   $s=$song;
		   echo '<div class="song">';
	       echo '<div class="text">';
		   echo '<p class="song_name">'.$s->getSongName().'</p>';
		   echo '<p class="title_buy">BUY</p>';
		   echo '</div>';
		   $imageArray=$so->getImageAccordingToId($s->getSongId());
		   $songImageArray=$imageArray;
		   echo '<div class="image">';
		   $source="../Images/Songs/".$songImageArray[0];
		   echo '<img src="'.$source.'" alt="'.$s->getSongName().'" width="674.5">';
		   echo '</div>';
		   echo '<div class="links">';
		   if($s->getITuneUrl()!=""){
			   echo '<p class="hvr-pop"><a href="'.$s->getITuneUrl().'">ITUNES</a></p>';
		   }
           if($s->getGooglePlayUrl()!=""){
			   echo '<p class="hvr-pop"><a href="'.$s->getGooglePlayUrl().'">GOOGLE PLAY</a></p>';
		   }
           if($s->getSpotifyUrl()!=""){
			   echo '<p class="hvr-pop"><a href="'.$s->getSpotifyUrl().'">SPOTIFY</a></p>';
		   }           
		   
		   echo '<form method="post">';
           echo '<p class="hvr-pop"><button type="submit" name="btn_lyric" value="'.$s->getSongId().'" id="buttons">LYRIC</button></p>';
		   echo '</form>';
           echo '</div>';
		   echo '</div>';
	   } 
